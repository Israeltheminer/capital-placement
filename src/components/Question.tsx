...
const FormLabel = ({ label }: { label: string }) => {
  return (
    <Typography
      variant="body1"
      style={{ fontSize: '12px', color: '#666666', marginLeft: '6px' }}
    >
      {label}
    </Typography>
  );
...
const Question = ({ type, detail }: Props) => {
  return (
    <li
      className="pt-[20px] mb-[20px]"
      style={{ maxHeight: detail.type ? '90px' : '70px' }}
    >
      {detail.type && (
      ...
      <div className="flex items-center justify-between">
        <p className="font-semibold text-base max-w-[320px]">
          {detail.question}
        </p>
        {!detail.default && (
          <div className="flex gap-7 items-center">
            ...
            {type !== 'additional' && (
              <FormControlLabel
                control={<AntSwitch />}
                label={<FormLabel label="Hide" />}
              />
            )}
            {type === 'additional' && (
              <img src="/pen.svg" alt="" className="scale-80" />
            )}
          </div>
        )}
      </div>
